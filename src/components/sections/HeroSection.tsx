...
const FloatingParticle = ({
  ...
  duration: number;
  delay: number;
}) => <motion.div className="absolute rounded-full bg-primary/[0.06]" style={{
  width: size,
  height: size,
  left: `${initialX}%`,
  top: `${initialY}%`
  ...
  return <section ref={heroRef} className="relative min-h-[100vh] flex items-center justify-center pt-20 pb-20 lg:pt-28 lg:pb-28 overflow-hidden">
      ...
      y: backgroundY
        ...
        ease: "easeOut"
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[80%]" style={{
          background: `radial-gradient(ellipse 90% 70% at 50% 15%, hsl(var(--primary) / 0.1) 0%, transparent 65%)`
        }} />
          
          {/* Secondary subtle accent */}
          <motion.div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] md:w-[1000px] md:h-[700px]" animate={{
          scale: [1, 1.015, 1],
          opacity: [0.7, 1, 0.7]
        }} transition={{
          duration: 10,
